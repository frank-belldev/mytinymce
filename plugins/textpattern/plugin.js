...
var textpattern = (function () {
  ...
  var $_avl0n4qrjfcjb6id = { getPatterns: getPatterns };
  ...
  var patternFromRng = function (patterns, rng, space) {
    ...
    var endPattern = $_65khuhqzjfcjb6iu.findEndPattern(patterns, text, rng.startOffset, delta);
    ...
    var startPattern = $_65khuhqzjfcjb6iu.findPattern(patterns, patternRng.toString());
  ...
  var applyBlockFormat = function (editor, patterns) {
    ...
    if (textBlockElm) {
      ...
      if (firstTextNode) {
        pattern = $_65khuhqzjfcjb6iu.findPattern(patterns, firstTextNode.data);
        if (!pattern) {
          return;
        }
  ...
  function handleEnter(editor, patterns) {
    ...
    wrappedTextNode = $_63dg71qwjfcjb6io.applyInlineFormatEnter(editor, patterns);
    if (wrappedTextNode) {
    ...
    $_63dg71qwjfcjb6io.applyBlockFormat(editor, patterns);
  }
  function handleInlineKey(editor, patterns) {
    var wrappedTextNode, lastChar, lastCharNode, rng, dom;
    wrappedTextNode = $_63dg71qwjfcjb6io.applyInlineFormatSpace(editor, patterns);
    if (wrappedTextNode) {
      dom = editor.dom;
  ...
  var setup = function (editor, patternsState) {
    ...
    editor.on('keydown', function (e) {
      if (e.keyCode === 13 && !global$2.modifierPressed(e)) {
        $_2mkeb8qvjfcjb6ij.handleEnter(editor, patternsState.get());
      }
    }, true);
    editor.on('keyup', function (e) {
      if ($_2mkeb8qvjfcjb6ij.checkKeyCode(keyCodes, e)) {
        $_2mkeb8qvjfcjb6ij.handleInlineKey(editor, patternsState.get());
      }
    });
    editor.on('keypress', function (e) {
      if ($_2mkeb8qvjfcjb6ij.checkCharCode(charCodes, e)) {
  ...
  var $_8xhmugqsjfcjb6if = { setup: setup };

  global.add('textpattern', function (editor) {
    var patternsState = Cell($_avl0n4qrjfcjb6id.getPatterns(editor.settings));
    $_8xhmugqsjfcjb6if.setup(editor, patternsState);
    return $_4u9hylqqjfcjb6ib.get(patternsState);
  });
  function Plugin () {
  }
